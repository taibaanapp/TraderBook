...
export const FinancialIndicators: React.FC<FinancialIndicatorsProps> = ({ symbol, theme, isExpanded, onToggle }) => {
  ...
  const TableRow = ({ label, keyName, unit = '', precision = 2, highlight = false }: any) => {
    ...
    const processedPeriods = currentData.map(calculateRatios).slice(0, 8);
    ...
    return (
      <tr className={cn(
        ...
        <td className={cn(
          "py-5 px-6 text-xs font-black uppercase tracking-widest sticky left-0 z-10 border-r min-w-[200px]",
        ...
        {processedPeriods.map((p, i) => {
          ...
          return (
            <td key={i} className="py-5 px-8 text-right">
              <div className="flex flex-col items-end">
                <span className={cn("text-base font-black tracking-tight", isDark ? "text-zinc-100" : "text-zinc-900")}>
                  {val !== null ? val.toLocaleString(undefined, { minimumFractionDigits: precision, maximumFractionDigits: precision }) : '-'}
                  {val !== null ? unit : ''}
                </span>
                {isBetter !== null && (
                  <span className={cn(
                    "text-[10px] font-black uppercase mt-1 flex items-center gap-1 tracking-widest",
  ...
  const calculateRatios = (period: any) => {
    ...
    const revenue = period.revenue ? period.revenue / 1000000 : null;
    const netIncome = period.netIncome ? period.netIncome / 1000000 : null;
  ...
  const currentDataForTable = data ? (view === 'annual' ? data.annual : data.quarterly) : [];
  const processedPeriodsForTable = currentDataForTable.map(calculateRatios).slice(0, 8);

  return (
    <div className={cn(
      "rounded-3xl border overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md",
      isDark ? "bg-zinc-900 border-zinc-800" : "bg-white border-zinc-200"
    )}>
      <button 
        onClick={onToggle}
        className={cn(
          "w-full p-6 flex items-center justify-between group transition-colors",
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-500">
          {loading ? (
            <div className="p-8 space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className={cn("h-12 rounded-xl animate-pulse", isDark ? "bg-zinc-800/50" : "bg-zinc-50")} />
              ))}
            </div>
          ) : data ? (
            <>
              <div className={cn("px-6 pb-6 pt-2 border-b flex flex-col md:flex-row md:items-center justify-between gap-4", isDark ? "border-zinc-800" : "border-zinc-100")}>
                <div className="flex items-center gap-4">
                  <div className={cn("flex p-1 rounded-lg border", isDark ? "bg-zinc-800 border-zinc-700" : "bg-zinc-100 border-zinc-200")}>
                    <button
                      onClick={(e) => { e.stopPropagation(); setView('annual'); }}
                      className={cn(
                        "px-6 py-1.5 text-xs font-black uppercase tracking-widest rounded-md transition-all",
                        view === 'annual' 
